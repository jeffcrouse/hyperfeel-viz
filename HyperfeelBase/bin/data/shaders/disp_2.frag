...
//	float fr = dot( eye, norm );// * .5 + .5;
	...
	float someVal = pow(fr, frExpo) * sin(ecPosition.x * noiseSurfaceSampleScale + sclZ ) * sin(ecPosition.y * noiseSurfaceSampleScale  + sclZ);

	
	gl_FragData[0] = vec4( vec3( someVal * someVal * 2. ), 1. );

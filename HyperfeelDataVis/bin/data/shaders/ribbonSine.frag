...
void main(void)
{
	float noiseSurfaceSampleScale = 10.;
	float fr = abs(dot( -ePos, norm ));
	
	float d = min(1., max(0., map( length(ecPosition.xyz), nearDepthScale, farDepthScale, 1., 0. )));
	
	float v1 = abs(cos(norm.x))*abs(cos(norm.y));
	
	float spec = pow(fr, 64.) * .2;
	
	float frp = pow(fr, frExpo);
	
	gl_FragColor = vec4( col * frp * v1 + spec, frp );

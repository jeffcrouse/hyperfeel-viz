...
void testApp::drawRussianDolls(){
	...
	for(int i=0; i<dollNodes.size(); i++){
		if( i == 0){
			float v1 = sin(elapsedTime * .8);
			float v2 = sin(elapsedTime * .4);
			dollNodes[i].setOrientation(ofVec3f( 0, v1 * pow(abs(v1),2.f) * 6, v2*pow(abs(v2),2.f)*30. ));
		}else{
...
void testApp::setupRussianDolls( float radians, float sphereRad ){
	...
	ofVec3f c;
	for(int i=0; i<dollNodes.size(); i++){
		c.set( ofRandom(.5,1.5), ofRandom(.5,1.5), ofRandom(.5,1.5) );
		c = (c * c).normalized() * 2.;
		dollColors[i].set(c.x,c.y,c.z);
